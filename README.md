...
#### Wald CI（渐近正态）
- **原理**：利用 MLE 的渐近正态性 $\hat{\lambda} \overset{\cdot}{\sim} N(\lambda, \lambda/n)$
- **优点**：计算简单，大样本下表现良好
...
### 双方装备类型差异（探索性分层，日度差分口径）

注：下表来自 Oryx 子类别累计列的日度差分，并对负修正值作 0 截断，适合展示方向和相对结构；由于 Oryx 会回溯修正分类，子类别合计可能高于最终累计列。正式引用最终累计数量时，应优先使用原始累计列。
...
| 2025–2026（近期，排除明显数据修正月后） | **约 0.65–0.8** | 多个月份出现乌方验证损失高于俄方；若直接纳入 2026-05 的负修正月，整个 2025–2026 加权比率约为 0.96 |
...
**偏误校正公式**: $\rho_{true} = \rho_{obs} \times (p_{UA} / p_{RU})$

| 场景 | 校正率比 | 含义 |
|------|---------|------|
| 等概率假设 | 2.07 | 率比不变 |
| 合理先验 (Bootstrap偏误校正, 中位数) | **1.63** | 在设定的 Beta 观测概率先验下仍>1 |
| 极端非对称 (p_RU=0.9, p_UA=0.4) | 0.92 | **率比逆转!** |
| **★ P(ρ_corrected > 1，条件于合理先验)** | **88.1%** | 这是先验敏感性分析结果，不是仅由数据直接识别出的概率 |
...
1. **双方影像验证损失率存在极显著差异（p < 0.0001）**：Oryx 双方日度 Change 数据显示俄方日均验证损失约为乌方的 2 倍（率比 2.07:1）。这是最稳健、可直接由数据复现的核心结论
2. **月度比率明显下降，近期多月出现乌方反超**：2022 年初期俄方损失远多于乌方；2025–2026 年若排除明显负修正月份，近期窗口比率约 0.65–0.8。若直接纳入全部修正值，2025–2026 加权比率约 0.96，因此应表述为“近期趋近或低于 1”，而不是无条件固定为 0.65
3. **偏误校正框架提供先验敏感性支持**：在设定的观测概率先验 $p_{RU}\sim Beta(7,3)$、$p_{UA}\sim Beta(5.5,4.5)$ 下，Bootstrap 给出 $P(\rho_{corrected} > 1)=88.1\%$。该结果依赖先验假设，应解读为稳健性分析而非纯数据识别出的后验事实 ⭐
4. **空间分布高度集中**：顿涅茨克州占俄方验证损失的 44%，最热网格 (48°N, 37°E) 单格 21.5%——揭示了战争的高度空间集中性 ⭐
5. **装备类型差异揭示作战方式，但具体数量需注明口径**：日度差分口径显示俄方在 AFV/IFV/坦克上损失远超（约 3-4x），乌方在 APC/MRAP 上反超；由于子类别存在回溯修正，具体数量引用时需说明口径
...
8. **官方声称数据显著高于影像验证数据**：主要重装备中，声称/验证比约为 2.6-29x；无人机等类别口径差异更大，不宜与重装备放在同一倍数范围内概括
