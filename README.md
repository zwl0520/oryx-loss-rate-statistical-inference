...
|------|---------|------|
| 等概率假设 | 2.07 | 率比不变 |
| 合理先验 (Bootstrap偏误校正, 中位数) | **1.63** | 校正后仍>1 |
| 极端非对称 (p_RU=0.9, p_UA=0.4) | 0.92 | **率比逆转!** |
| **★ P(ρ_corrected > 1 \| 合理先验)** | **88.1%** | 校正后仍九成概率俄方更高 |
...
3. **偏误校正框架支持结论稳健性**：基于 Bootstrap 的偏误校正方法给出 $P(\rho_{corrected} > 1) = 88.1\%$——即使在考虑观测偏误后，俄方真实损失率高于乌方的概率仍接近九成 ⭐
