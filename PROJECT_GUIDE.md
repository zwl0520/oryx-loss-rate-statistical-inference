...
**核心发现**: 逆转条件为 $p_{UA}/p_{RU} \leq 0.485$；在设定的 Beta 观测概率先验下，偏误校正后率比中位数 1.63，$P(\rho_{corrected} > 1) = 88.1\%$。这是先验敏感性分析结果，不是仅由观测数据直接识别出的概率。
...
### 装备类型差异（按 RU/UA 比率排序，探索性口径）

注：下表使用 Oryx 子类别累计列差分后的日度口径，并对负修正值作 0 截断。它适合说明装备类型差异方向，但具体数量可能高于原始最终累计列；正式引用数量时要说明该口径。
...
| 2025–2026 | **约 0.65–0.8（排除明显负修正月后）** | 近期多个月份乌方反超；若纳入全部修正值，2025–2026 加权比率约 0.96 |
...
**Q8: 月度比率下降到接近或低于 1 意味着什么？**

A: 反映了战争性质的深刻转变：
- 2022 年初：俄军作为进攻方，大规模装甲纵队突入敌方纵深 → 极高消耗率
- 2025-2026 年：战争进入长期消耗战，乌方承受越来越大压力，多个近期月份的 Oryx 验证损失已高于俄方
- 注意口径：0.65 是排除明显负修正月份后的近期窗口统计；若把 2026-05 的负修正也直接纳入，2025-2026 加权比率约为 0.96
- 这对双方战争的可持续性有直接政策含义
...
- [ ] 能解读月度比率下降到接近或低于 1 的含义，并说明负修正月份对 0.65 口径的影响
