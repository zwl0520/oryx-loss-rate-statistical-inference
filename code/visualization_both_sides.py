...
print("[10/16] Both sides time series...")
fig, axes = plt.subplots(3, 1, figsize=(14, 13))

ax = axes[0]
for side, color, label in [('Russia_Total', '#c0392b', 'Russia'), ('Ukraine_Total', '#2980b9', 'Ukraine')]:
...
ax.plot(daily['Date'], daily['Russia_Total'].cumsum(), color='#c0392b', linewidth=1.5, label=f'Russia ({int(daily["Russia_Total"].sum()):,})')
ax.plot(daily['Date'], daily['Ukraine_Total'].cumsum(), color='#2980b9', linewidth=1.5, label=f'Ukraine ({int(daily["Ukraine_Total"].sum()):,})')
...
ax.axhline(y=1.0, color='black', linestyle='--', linewidth=1, label='Ratio = 1')
...
ax.set_title('Monthly Equipment Loss Ratio: Russia / Ukraine')
...
print("[11/16] Both sides CI forest plot...")
...
print("[12/16] Equipment type comparison...")
...
w = 0.35
ax.bar(x - w/2, ru_eq_vals, w, color='#c0392b', alpha=0.8, label='Russia', edgecolor='white')
ax.bar(x + w/2, ua_eq_vals, w, color='#2980b9', alpha=0.8, label='Ukraine', edgecolor='white')
ax.set_xticks(x)
...
ax.set_ylabel('Loss Ratio RU/UA')
ax.set_title('Equipment Loss Ratio by Type')
...
print("[13/16] Both sides status comparison...")
...
for ax, prefix, title in [(axes[0], 'Russia_', 'Russia'), (axes[1], 'Ukraine_', 'Ukraine')]:
    ...
    ax.pie(vals, labels=status_labels, autopct='%1.1f%%', colors=status_colors,
...
print(f"\nBoth sides visualization complete! Charts 10-13.")
