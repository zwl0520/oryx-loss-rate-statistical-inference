...
def macro_region(lat, lon):
    if pd.isna(lat) or pd.isna(lon): return 'No Coordinates'
    if lon >= 36: return 'East'
    if lon < 28: return 'West'
    if lat >= 50: return 'North'
    if lat < 47.5: return 'South'
    return 'Central'

ws_detail['region'] = ws_detail.apply(lambda r: macro_region(r['latitude'], r['longitude']), axis=1)
...
ax.set_title('Equipment Losses by Macro Region\n(WarSpotting, image-verified)')
...
ax.set_title('Top 10 Loss Locations by Nearest Settlement')
for i, (bar, val) in enumerate(zip(ax.patches, area_counts.values[::-1])):
...
ax.set_xlabel('p_observed_Ukraine (assumed)')
ax.set_ylabel('p_observed_Russia (assumed)')
ax.set_title('Corrected RU/UA Ratio\n(observed = 2.06)')
# 标注
...
ax.axhline(y=1.0, color='black', linestyle='--', linewidth=2, label='Ratio = 1 (equal)')
...
ax.set_title('Ratio Reversal Boundary\n(below dashed line = Ukraine losses exceed Russia)')
...
ax.set_xlabel('Rate per Day λ')
ax.set_ylabel('Density')
ax.set_title(f"Weekly Block Bootstrap Distribution (B={int(boot_summary['bootstrap_reps'].values[0])}, "
             f"n_weeks={int(boot_summary['n_weeks'].values[0])})")
ax.legend(fontsize=8)

# 右：分类型 Bootstrap SE 对比
ax = axes[1]
