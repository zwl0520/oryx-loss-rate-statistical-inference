...
俄乌冲突中，双方公布的战报数据差异极大，如何客观估计双方真实装备损失率并检验其统计显著性是冲突数据分析的核心挑战。本文针对选题 T1 的两个核心统计问题——①如何给出准确的装备损失率点估计与区间估计？②双方损失率是否存在统计显著差异？——利用开源情报平台 Oryx 提供的双方经影像验证的装备损失记录（俄方 23,556 件，乌方 11,397 件，2022 年 2 月至 2026 年 6 月），结合乌克兰总参谋部官方战报数据（1,557 天累计），构建了从"估计偏误到区间估计"的完整统计推断链条。具体而言：本文基于 Poisson 假设，采用极大似然估计（MLE）获得日均损失率点估计，结果俄方 $\hat{\lambda}_{RU}=15.11$ 件/天、乌方 $\hat{\lambda}_{UA}=7.31$ 件/天，率比 $\hat{\rho}=2.07$（95% CI: [2.02, 2.11]）；通过 Wald、Score 和 Exact（Garwood, 1936）三种方法构建置信区间，Monte Carlo 模拟表明 Exact CI 在小 $\lambda$ 下覆盖率最优（$\lambda$=0.5 时 96.6%）；两独立 Poisson 条件精确检验（E-test）确认双方损失率差异极显著（p < 0.0001）；Block Bootstrap（b=11）SE 估计 0.99 是 IID Bootstrap（0.42）的 2.4 倍，表明忽略时间依赖性将严重低估不确定性。**在偏误校正方面**，本文构建了观测概率偏误校正框架：利用官方声称数据约束俄方观测概率下界（坦克 p $\geq$ 0.34、装甲车辆 p $\geq$ 0.52）；通过 Block Bootstrap 与观测概率先验（Beta 分布）结合的偏误校正方法，获得校正后率比中位数 1.63（95% CI: [0.67, 3.44]），并计算出在合理先验下 $P(\rho_{corrected} > 1) = 88.1\%$——即考虑观测偏误校正后，俄方真实损失率高于乌方的后验概率仍接近九成。进一步的分层分析揭示：（1）双方月度损失比率从 2022 年初的 3.66:1 逆转为近期的 0.65:1；（2）装备类型差异显著——俄方在进攻型装备上损失远超乌方（AFV 4.49x、IFV 4.08x、坦克 3.14x），乌方仅在 APC（0.56x）上反超；（3）官方声称与影像验证之间存在 3–29 倍系统性偏差。本文的分析框架——从 Poisson MLE 到 CI 比较、Block Bootstrap 时间依赖性校正、E-test 精确检验，再到基于 Bootstrap 的观测概率偏误校正——为该选题提供了严格、可复现的统计回答，对冲突数据的客观推断具有方法论参考价值。
...
**声称数据约束观测概率下界**：为缩小 $p$ 的可行域，本文利用乌克兰官方战报数据构造一个弱约束。假设官方声称不会系统性低估敌方损失（即声称数 $\geq$ 真实数），则：
...
**基于 Bootstrap 的偏误校正推断**：将观测概率视为随机变量（先验 $p_{RU} \sim Beta(7,3)$，均值 0.70；$p_{UA} \sim Beta(5.5, 4.5)$，均值 0.55），嵌入 Block Bootstrap（$B=5000$）框架进行偏误校正推断。结果如下：

- 偏误校正后率比中位数：**1.63**（观测值 2.07 的 79%）
- 偏误校正后 95% Bootstrap CI：**[0.67, 3.44]**
- 核心稳健性指标：**$P(\rho_{corrected} > 1) = 88.1\%$**
...
核心结论：**观测到的 2.07:1 率比在合理的观测偏误先验下具有较高的稳健性，$P(\rho_{corrected} > 1) = 88\%$。仅当假设俄方装备几乎完全被观测（$p_{RU} \approx 0.9$）且乌方装备观测率极低（$p_{UA} \approx 0.4$）时，偏误校正后的率比才会降至 1 以下。**考虑到俄占区的影像获取难度确实更大，真实的率比大概率在 1.0–2.0 之间，但俄方损失率高于乌方这一方向性结论在合理假设下得到偏误校正框架的支持。
...
2. **偏误校正框架支持结论的稳健性**：通过将观测概率先验嵌入 Block Bootstrap 的偏误校正方法，校正后率比中位数为 1.63（95% CI: [0.67, 3.44]），$P(\rho_{corrected} > 1) = 88.1\%$——在合理先验下，俄方真实损失率高于乌方的概率接近九成。仅当乌方观测概率系统性低于俄方 51.5% 时才可能逆转
3. **双方损失比率呈现"从 >3 到 <1"的时序逆转**：2022 年初月均比率 3.66（俄方多损），2025-2026 年降至 0.65（乌方反超）
4. **装备类型差异极显著且有战术含义**：俄方在进攻型装备（AFV 4.49x、IFV 4.08x、坦克 3.14x）上损失远超乌方，偏误校正分析表明这些类别的结论对观测偏误具有极高稳健性（逆转需 $p_{UA}/p_{RU} < 0.25$）；乌方仅在 APC（0.53x）、MRAP（0.07x）等防御/运输装备上反超
